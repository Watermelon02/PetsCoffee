    ...
    private void login() {
        {//进行登陆判断
            ...
            Archive.loadCoffee(this, account, user -> {
                List<CoffeeShop> coffeeShops = CoffeeDatabase.getInstance(this).coffeeShopDao().queryCoffees();
            ...
            Archive.loadCoffee(this, register_account, coffeeShop -> {
                if (coffeeShop == null) {
                    ...
                    if (register_password.equals(register_confirm)) {//检测两次密码输入是否相同
                        CoffeeShop register_coffeeShop = new CoffeeShop(1, 0, 10000.0f);
                        register_coffeeShop.setAccount(register_account);
                        register_coffeeShop.setName(register_name);
                        register_coffeeShop.setPassword(register_password);
