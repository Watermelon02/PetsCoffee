...
public class Eater {// 实现多宠物同时开饭
    ...
    public Eater(Context context, PetsAdapter.PetViewHolder viewHolder, List<Goods> bag, Pets pet) {
        ...
        if (Bag.search(bag, "Foods") != -1) {
            //如果背包里有食物
            alert.setTitle("投 喂").setMessage("剩余：" + bag.get(Bag.search(bag, "Foods")).getNumber())
                    .setCancelable(false)
                    .setView(view)
                    .setPositiveButton("确认", (dialog, which) -> {
                        TextView petHungerText = viewHolder.getPetsHunger();//
                        int eaten = Integer.parseInt(editText.getText().toString());//获取用户输入的投喂食物数
                        ...
                        if (eaten <= total) {//如果输入的数量小于/等于剩余食物数量
                            if (eaten <= 10 - pet.getHunger()) {//吃的食物数量小于/等于未满的饥饿值
                                pet.setHunger(pet.getHunger()+eaten);
                                bag.get(Bag.search(bag, "Foods")).setNumber(-eaten);
                                petHungerText.setText("hunger:：" + pet.getHunger());//修改宠物界面饱食度
                                Toast.makeText(context, "恢复了" + eaten + "点饱食度", Toast.LENGTH_SHORT).show();
                            } else {//吃的食物数量大于未满的饥饿值
